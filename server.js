...
const views={
  wado_metadata:
  {'map':"function(doc){var key={};if(doc.dataset){studyUID=doc.dataset['0020000D'].Value;"+
    " seriesUID=doc.dataset['0020000E'].Value;"+
    " instanceUID=doc.dataset['00080018'].Value;"+
    " for(var t in doc.dataset){var fallback='';if(doc.dataset[t]&&doc.dataset[t].Value){key[t]={};if(doc.dataset[t].Value!==''){if(doc.dataset[t].vr==='PN')"+
    " key[t].Value=[{'Alphabetic':doc.dataset[t].Value||fallback}];else key[t].Value=[doc.dataset[t].Value||fallback]}"+
  ...
  {'map':"function(doc){var tags=[['institution','00080080','','CS'],['patientID','00100020','','LO'],['patientName','00100010','','PN']"+
    ...
    " if(doc.dataset){var i;for(i=0;i<tags.length;i++){var tag=tags[i];var name=tag[0];var t=tag[1];var fallback=tag[2];var vr=tag[3];if(doc.dataset[t]&&doc.dataset[t].Value){key[t]={};"+
    " if(doc.dataset[t].Value!==''){if(vr==='PN')"+
    " key[t].Value=[{'Alphabetic':doc.dataset[t].Value||fallback}];else key[t].Value=[doc.dataset[t].Value||fallback]}"+
  ...
  qido_study_series:
  {'map':"function(doc){if(doc.dataset){studyUID=doc.dataset['0020000D'].Value;"+
    " seriesUID=doc.dataset['0020000E'].Value;"+
  ...
  {'map':"function(doc){var studyTags=[['charset','00080005','','CS',1],['studyDate','00080020','','DA',1],"+
    ...
    " ['numberOfStudyRelatedInstances','00201208','','IS',1],['retrieveAETitle','00080054','','AE']];var studyKey={};if(doc.dataset){studyUID=doc.dataset['0020000D'].Value;"+
    " var i;for(i=0;i<studyTags.length;i++){var tag=studyTags[i];var name=tag[0];var t=tag[1];var fallback=tag[2];var vr=tag[3];var required=tag[4];if(doc.dataset[t]&&doc.dataset[t].Value){if(doc.dataset[t].Value!==''){studyKey[t]={};if(vr==='PN')"+
    " studyKey[t].Value=[{'Alphabetic':doc.dataset[t].Value||fallback}];else studyKey[t].Value=[doc.dataset[t].Value||fallback]}"+
  ...
  {'map':"function(doc){var seriesTags=[['charset','00080005','','CS',1,],['modality','00080060','','CS',1],['timezoneOffsetFromUTC','00080201','','SH',0],['seriesDescription','0008103E','','LO',0],['retrieveURL','00081190','','UR',0],"+
    ...
    " var seriesKey={};if(doc.dataset){studyUID=doc.dataset['0020000D'].Value;"+
    " seriesUID=doc.dataset['0020000E'].Value;"+
    " var i;for(i=0;i<seriesTags.length;i++){var tag=seriesTags[i];var name=tag[0];var t=tag[1];var fallback=tag[2];var vr=tag[3];var required=tag[4];if(doc.dataset[t]&&doc.dataset[t].Value){seriesKey[t]={};if(doc.dataset[t].Value!==''){if(vr==='PN')"+
    " seriesKey[t].Value=[{'Alphabetic':doc.dataset[t].Value||fallback}];else seriesKey[t].Value=[doc.dataset[t].Value||fallback]}"+
  ...
  {'map':"function(doc){var tags=[['charset','00080005','','CS',1],['SOPClassUID','00080016','','UI',1],['SOPInstanceUID','00080018','','UI',1],['instanceAvailability','00080056','','CS',1],['timezoneOffsetFromUTC','00080201','','SH',0],"+
    ...
    " ['seriesUID','0020000E','','UI',1],['retrieveAETitle','00080054','','AE',1]];var key={};if(doc.dataset){studyUID=doc.dataset['0020000D'].Value;seriesUID=doc.dataset['0020000E'].Value;instanceUID=doc.dataset['00080018'].Value;var i;"+
    " for(i=0;i<tags.length;i++){var tag=tags[i];var name=tag[0];var t=tag[1];var fallback=tag[2];var vr=tag[3];var required=tag[4];if(doc.dataset[t]&&doc.dataset[t].Value){key[t]={};if(doc.dataset[t].Value!==''){if(vr==='PN')"+
    " key[t].Value=[{'Alphabetic':doc.dataset[t].Value||fallback}];else key[t].Value=[doc.dataset[t].Value||fallback]}"+
